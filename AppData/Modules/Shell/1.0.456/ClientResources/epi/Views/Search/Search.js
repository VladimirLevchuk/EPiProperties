...
(function ($, epi) {
    epi.search = function () {
        ...
        var _createToolTip = function (searchResult) {
            ...
            if (searchResult) {
                ...
                for (var toolTipIndex in searchResult.ToolTipElements) {
                    ...
                    results.push("<tr>");
                    results.push("<th>" + toolTipElement.Label + "</th>");
                    results.push("<td>" + toolTipElement.Value + "&nbsp;</td>");
                    results.push("</tr>");
                }
                ...
                if (typeof (searchResult.PreviewText) === "string" && searchResult.PreviewText.length > 0) {
                    results.push("<tr>");
                    results.push("<th>");
                    results.push(epi.shell.resource.get("episerver.shell.ui.views.search.resources", "previewtext"));
                    results.push("</th>");
                    results.push("<td>" + searchResult.PreviewText + "</td>");
                    results.push("</tr>");
                ...
                if (typeof (searchResult.Language) === "string" && searchResult.Language !== "") {
                    results.push("<tr>");
                    results.push("<th>");
                    results.push(epi.shell.resource.get("episerver.shell.ui.views.search.resources", "language"));
                    results.push("</th>");
                    results.push("<td>" + searchResult.Language + "</td>");
                    results.push("</tr>");
                }
                ...
                if (results.length > 2) {
                    return "<div class='epi-toolTip'>" + results.join("") + "</div>";
                }
                else {
                    return "";
                }
        ...
        pub.performSearch = function (e) {
            ...
            encodedData[0].value = $("<div/>").text(encodedData[0].value).html();
            ...
            if (typeof (data) === "object" && typeof (data.providerId) === "string") {
                ...
                if (results.length > 0) {
                    ...
                    for (var i = 0; i < results.length; i++) {
                        var html = "<li>";
                        html += "<a href='" + results[i]["Url"] + "' class='" + results[i]["IconCssClass"] + "'>" + results[i]["Title"];
                        var previewText = results[i]["PreviewText"]
                        ...
                        html = $(html).appendTo(resultContainer);

                        //Attach the hover event on the link, a click event to enable custom client side handling and the custom search data.
                        html.find("a:first")
